...
class Line:
    def __init__(self, s = ''):
        ...
        self.content_tuples = self.get_content_tuples()
    ...
    def get_nouns(self):
        ...
        return [n[0] for n in self.content_tuples if 'NN' in n[1]]
    ...
    def get_proper_nouns(self):
        ...
        return [n[0] for n in self.content_tuples if ('NNP' in n[1] and n[0][0].isupper())]
    ...
    def get_people(self):
        ...
        excluded = ['(',')'] #unwanted in list. Parentheses are tagged with 'NNP'
        nnp = [n for n in self.tagged if ('NNP' in n[1] and n[0] not in excluded and n[0][0].isupper())] #list of nouns in line (second entry in tuple gives tag)
        #checks to see if it's tagged with 'NNP', isn't a paren, and is capitalized
...
def consolidate_tokens(l,s):
    """Takes in a list of tuples, and the original line. Checks that they're chunked properly.
    ...
    nl = [n[0] for n in l] #just the nouns
    ...
    #below, temp takes from the list of strings, nl.
    #The original list, l, is used to ensure tokens with the same tag are chunked
    ...
    while x < len(l):
        ...
        while x + i + 1 < len(l): #while the next "Persons" after them also obinly have one part of a name
            ...
            #AND the POS tags' beginnings match one another (e.g. 'NNP' and 'NN')
